...
# Build, test, inspect, and collect all nineteen current artifacts
...
- Recipe JSONs available in the selected capability tier are present in the data pack and gated during server recipe lookup. The build selects both the version-correct JSON schema and resource directory: 1.20.5/1.20.6 use plural `recipes/` with ingredient objects and `result.id`, while later audited targets use the schema required by their own release. Later-version variants are removed from older JARs. Data-pack inspection alone does not indicate whether a bundled recipe is currently enabled.
- The support catalog is complete for today's 35 stable Fabric Carpet coordinates and 43 exact Minecraft releases, but it is not yet the same as 43 release-ready builds. Nineteen exact Minecraft releases have current adapters; the remaining matrix rows stay `released-legacy` or `planned` until their own versioned sources pass build and behavior checks.
- Minecraft 1.20.5–1.20.6 and 1.21–1.21.11 are deliberately marked `build-only`: their current adapters pass unit, packaging, JAR, and server checks, but they do not yet have the 26.x gameplay GameTest coverage.
