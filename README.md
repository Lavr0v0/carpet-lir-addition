...
# Build, test, inspect, and collect all twenty-nine current artifacts
...
- The support catalog is complete for today's 35 stable Fabric Carpet coordinates and 43 exact Minecraft releases, but it is not yet the same as 43 release-ready builds. Twenty-nine exact Minecraft releases have current adapters; the remaining 14 matrix releases stay `planned` until their own versioned sources pass build and behavior checks.
- Minecraft 1.19–1.19.4, 1.20–1.20.6, and 1.21–1.21.11 are deliberately marked `build-only`: these 24 current adapters pass unit, packaging, JAR, and server checks, but they do not yet have the 26.x gameplay GameTest coverage.
