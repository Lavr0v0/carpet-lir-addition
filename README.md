...
The builds pin both compilation and runtime to the Java generation required by each Minecraft line: Java 8 for 1.14.4, Java 16 for 1.17–1.17.1, Java 17 for 1.18–1.20.4, Java 21 for 1.20.5–1.21.11, and Java 25 for 26.x. Gradle resolves toolchains automatically when necessary, and the disposable server harness rejects a run whose Fabric log reports the wrong Java major version. The first build therefore needs network access, and a real 1.14.4 server smoke test must itself run on Java 8 because that generation's Mixin runtime cannot parse modern host classes.
...
# Build, test, inspect, and collect all thirty-four current artifacts
