...
# Build one current Yarn adapter
...
# Build, test, inspect, and collect all thirteen current artifacts
...
- The support catalog is complete for today's 35 stable Fabric Carpet coordinates, but it is not yet the same as 35 release-ready builds. Thirteen exact Minecraft targets have current adapters; the remaining matrix rows stay `released-legacy` or `planned` until their own versioned sources pass build and behavior checks.
- Minecraft 1.21.4–1.21.11 are deliberately marked `build-only`: their current adapters pass unit, packaging, JAR, and server checks, but they do not yet have the 26.x gameplay GameTest coverage.
