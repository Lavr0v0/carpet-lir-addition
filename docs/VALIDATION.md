...
Use `./gradlew test` for fast rule/resource integrity tests and `./gradlew runGameTest` for actual 26.x server behavior. The 26.x GameTest suite covers the highest-risk positive, disabled, permission, environmental, snowy-state, and Silk Touch loot-context paths. Minecraft 1.14.4 must be smoke-tested on a Java 8 server. The `scripts/smoke-test-modern-server.ps1` harness uses a GUID-isolated disposable run directory to exercise rule-on, snowy-state, rule-off, spectator, live recipe fallback, piston-broken budding-amethyst no-drop/drop paths, and shutdown behavior for Minecraft 1.19–1.19.4, 1.20–1.20.6, and 1.21–1.21.11, waits for its fake player to finish logging in, then removes only that generated directory with `-CleanRunDirectory`.

Enhanced modern smokes on exact Minecraft 1.19–1.20.1 plus the representative 1.20.2, 1.20.6, and 1.21.10 boundaries deliberately cover both `RecipeManager` lookup shapes. They first cache the controlled gravel-to-tuff recipe, install a later-ordered disposable fallback data pack while the server is still running, wait for recipe reload, and prove that an enabled direct lookup still produces tuff. After disabling `renewableTuff`, they confirm that the cached path produces fallback cobblestone and that the same direct `loot ... furnace_smelt` lookup now falls through to cobblestone. The 1.19–1.20.1 runs exercise the pre-entry Pair API; the 1.20.2 boundary run exercises the entry Pair API shared by 1.20.2–1.20.4. All five 1.19 runs also exercise the mangrove recipe resource, disabled/enabled reinforced-deepslate hardness, three real fake-player Silk Touch mining paths, and Warden rule-off, `doMobLoot=false`, and enabled 1–4-drop paths. The 1.19/1.19.1 `/kill` checks cover the fallback death Mixin; 1.19.2–1.19.4 cover Fabric's server death event. The enabled baseline prevents a native fallback-first iteration order from creating a false positive. Release acceptance uses `-CleanRunDirectory` to remove the GUID world and temporary data pack, preventing cross-version state and large test worlds from accumulating. The 24 Minecraft 1.19–1.19.4, 1.20–1.20.6, and 1.21–1.21.11 targets stay `build-only` until equivalent full gameplay automation is ported. The remaining manual checks above are release acceptance checks, particularly recipe shapes and in-world interaction feel.
...
| --- | --- | --- | --- | --- |
| 1.19–1.20.4 | plural `data/carpetlir/recipes/` | `{ "item": ... }` or `{ "tag": ... }` objects | `result.item` object | string item id |
...
The JAR audit rejects a resource that lands in the wrong directory or uses the wrong ingredient/result shape even if another Minecraft generation would accept that JSON. Minecraft 1.19–1.20.1 use a pre-entry Pair-cache adapter, while 1.20.2–1.20.4 use the entry-based Pair adapter and compatible mechanics remain in shared source layers. Across the exact 1.19 line, the audit also enforces the selected old/new Carpet Rule annotation, `fabric`/`fabric-api` dependency id, legacy/registry fluid-tag adapter, and mutually exclusive death-Mixin/event path; the wrong-side class or server-active Mixin registration is rejected.
