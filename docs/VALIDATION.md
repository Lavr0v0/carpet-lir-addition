...
Representative modern smokes on Minecraft 1.20.2, 1.20.6, and 1.21.10 deliberately cover both `RecipeManager` lookup shapes. They first cache the controlled gravel-to-tuff recipe, install a disposable fallback data pack while the server is still running, reload recipes, disable `renewableTuff`, and confirm that the cached Pair path now produces the fallback cobblestone result. A separate `loot ... furnace_smelt` check covers the direct three-argument fallback path. Cleanup removes the GUID world and the temporary data pack, preventing cross-version state and large test worlds from accumulating. The seventeen Minecraft 1.20.2–1.20.6 and 1.21–1.21.11 targets stay `build-only` until equivalent full gameplay automation is ported. The remaining manual checks above are release acceptance checks, particularly recipe shapes and in-world interaction feel.
