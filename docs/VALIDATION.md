...
Use `./gradlew test` for fast rule/resource integrity tests and `./gradlew runGameTest` for actual 26.x server behavior. The 26.x GameTest suite covers the highest-risk positive, disabled, permission, environmental, snowy-state, and Silk Touch loot-context paths. Minecraft 1.14.4 must be smoke-tested on a Java 8 server. The `scripts/smoke-test-modern-server.ps1` harness uses a GUID-isolated disposable run directory to exercise rule-on, snowy-state, rule-off, spectator, live recipe fallback, and shutdown behavior for Minecraft 1.20.2–1.20.6 and 1.21–1.21.10, waits for its fake player to finish logging in, then removes only that generated directory with `-CleanRunDirectory`.

Representative modern smokes on Minecraft 1.20.2, 1.20.6, and 1.21.10 deliberately cover both `RecipeManager` lookup shapes. They first cache the controlled gravel-to-tuff recipe, install a later-ordered disposable fallback data pack while the server is still running, reload recipes, and prove that an enabled direct lookup still produces tuff. After disabling `renewableTuff`, they confirm that the cached path produces fallback cobblestone and that the same direct `loot ... furnace_smelt` lookup now falls through to cobblestone. Minecraft 1.20.2–1.20.4 specifically exercise the Pair-returning cached API. The enabled baseline prevents a native fallback-first iteration order from creating a false positive. Release acceptance uses `-CleanRunDirectory` to remove the GUID world and temporary data pack, preventing cross-version state and large test worlds from accumulating. The seventeen Minecraft 1.20.2–1.20.6 and 1.21–1.21.11 targets stay `build-only` until equivalent full gameplay automation is ported. The remaining manual checks above are release acceptance checks, particularly recipe shapes and in-world interaction feel.
...
| --- | --- | --- | --- | --- |
| 1.20.2–1.20.4 | plural `data/carpetlir/recipes/` | `{ "item": ... }` or `{ "tag": ... }` objects | `result.item` object | string item id |
