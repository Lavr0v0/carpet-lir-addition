...
Enhanced modern smokes on exact Minecraft 1.19.3–1.20.1 plus the representative 1.20.2, 1.20.6, and 1.21.10 boundaries deliberately cover both `RecipeManager` lookup shapes. They first cache the controlled gravel-to-tuff recipe, install a later-ordered disposable fallback data pack while the server is still running, wait for recipe reload, and prove that an enabled direct lookup still produces tuff. After disabling `renewableTuff`, they confirm that the cached path produces fallback cobblestone and that the same direct `loot ... furnace_smelt` lookup now falls through to cobblestone. The 1.19.3–1.20.1 runs exercise the pre-entry Pair API; the 1.20.2 boundary run exercises the entry Pair API shared by 1.20.2–1.20.4. The 1.19.3/1.19.4 runs also exercise the mangrove recipe resource, disabled/enabled reinforced-deepslate hardness, three real fake-player Silk Touch mining paths, and Warden rule-off, `doMobLoot=false`, and enabled 1–4-drop paths. The enabled baseline prevents a native fallback-first iteration order from creating a false positive. Release acceptance uses `-CleanRunDirectory` to remove the GUID world and temporary data pack, preventing cross-version state and large test worlds from accumulating. The 21 Minecraft 1.19.3–1.19.4, 1.20–1.20.6, and 1.21–1.21.11 targets stay `build-only` until equivalent full gameplay automation is ported. The remaining manual checks above are release acceptance checks, particularly recipe shapes and in-world interaction feel.
